...
class PPHStat:
    def __init__(self,dataloc = './'):
            self.dataloc = loc(dataloc)
            self.nodes = pd.read_csv(dataloc + "fidesz_nodes.csv")[
            ["ID","name", "address", "org_relations","person_relations"]]
            self.edges = pd.read_csv(dataloc + "fidesz_edges.csv")[
            ["Source","Target","id","name","node1_role", "node2_role","start_time","end_time"]]
    #tulajlista function
    
    
    def doshit(self):
        self.edges=self.edges[["Source","Target","id","name","node1_role", "node2_role","start_time","end_time"]]
        ownership(self.edges,self)[["Source","Target","id","name","node1_role", "node2_role"]].to_csv(self.dataloc + "P-O relations.csv", index=False)
        #Search for P-P relations

        self.friend_edges=friendship(self.edges)
        

    def doothershit(self):
        self.friend_ids=self.friend_edges["Source"],self.friend_edges["Target"]
        self.halo = pd.read_csv(self.dataloc + 'list_firms.csv',index_col=0,header=None)[1]
        self.kozbesz = pd.read_csv(self.dataloc + 'list_kozbesz.csv',index_col=0,header=None)[1]
        self.ck = self.kozbesz.apply(cleanfirm).unique()
        self.ch = self.halo.apply(cleanfirm).unique()
        
        self.df = split_lev(self.ch,self.ck,10,0.8)
        self.df[self.df['rate'] > 0.9].sort_values('rate').to_csv(self.dataloc + "mapping.csv")

        ##here is the main process
        self.part  = pd.read_csv(self.dataloc + 'part.csv')
        self.win_final = pd.read_csv(self.dataloc + 'winner_resolved_final.csv')
        self.p_o = pd.read_csv(self.dataloc + 'P-O relations.csv')
        self.fidesz_nodes = pd.read_csv(self.dataloc + 'fidesz_nodes.csv')
        self.mapping = pd.read_csv(self.dataloc + 'mapping.csv')



        self.final_kozbesz3 = preproc (self.part, self.win_final)
        ...
        self.merged_po2['name_x'] = self.merged_po2['name_x'].apply(cleanfirm)
        ...
        for i in self.list_name_y:
            self.filtered_po = self.merged_po2[self.merged_po2['name_y'] == i]
            self.kuka=pd.DataFrame(list(set(self.filtered_po['name_x'])))
            self.kuka["id"]=[i for x in range(len(self.kuka))]
            self.dict_name_y=self.dict_name_y.append(self.kuka, ignore_index=True)
        del self.kuka
        ...
        self.final_kozbesz3['NAME'] = self.final_kozbesz3['NAME'].apply(cleanfirm)
...
def ownership(df,self):
    ...
    #filter by ownership
    viszony=pd.read_csv(self.dataloc + "viszony.csv")
...
def stats(df,start,end,merged_po2):
